...
<input class="fac-current-category" type="hidden" data-product-categoryname="<#if breadcrumbSubCat??>${breadcrumbSubCat}</#if>" />
...
<input type="hidden" id="shareOnContactFormLink" value="${shareContactFormLink}" />
<input type="hidden" id="EnquiryWithUsLable" value="${contactUsEnquiryWithUs}" />
...
<@fmt.message key="shop-now-pdp-product-video-title" var="productVideoTitle"/>
<@fmt.message key="shop-now-pdp-promo-banner" var="promoBanner"/>
<@fmt.message key="shop-now-pdp-promo-banner-URL" var="promoBannerURL"/>
...
<input type="hidden" id="pdpLocaleValue" value="${locale}" />
...
<#if product??>
    ...
    <input type="hidden" id="brandNameAndShortdescription" value="${brandName} ${shortDescription?html}" />
	...
	<section onload="loadImage();" class="c-koh-product-details v-koh-default c-koh-product-details-ap" >
	   ...
	   <div onload="loadImage();" class="koh-product-top-row" >
		            ...
		            <div class="product-images__navigation">
		               <#if imgInitialViewUrl?? && imgZoomViewUrl??>
			               <div class="product-slide product-slide-first">
			                  <img class="koh-product-iso-image" src="${imgInitialViewUrl}" data-magnify-src="${imgZoomViewUrl}" width="1800" height="800" alt="${product.defaultSKU}">
			               </div>
		               </#if>
	      ...
	      <div class="koh-product-details">
	               ...
	               <#if product.skus?? && product.skus[0]?has_content>
		               <#list product.skus as sku>
			               <#if sku.skuId == defaultSwatchId>
				               ...
				               <#if sku.colorFinishName?has_content>${sku.colorFinishName}</#if></span>
			               </#if>		  
		               </#list>
	               </#if>
	               <ul>
			                  ...
			                  <#list product.skus as skuItem>
								  ...
								  <#assign imgswatchUrl = imgswatch?replace("{0}","${imgSwatch}") />
			                  	  ...
			                  	  <#else>
					                  ...
					                  <#if skuItem.colorFinishName?has_content>
					                  		<#assign skuItemColorFinishName = skuItem.colorFinishName />
					                  </#if>
									  <#assign productPriceNumber = ""/>
									 ...
									 <#assign bigcommerceUrl ="" />
									 <#list skuItem.retailerUrlMap?keys as urlMapKey>
										<#if urlMapKey == "RETAILER_4" && skuItem.retailerUrlMap[urlMapKey]?? && skuItem.retailerUrlMap[urlMapKey]?has_content>
				                  			<#assign bigcommerceUrl = skuItem.retailerUrlMap[urlMapKey] />
										</#if>
									 </#list>
									 
						                  <li>
						                     <span class="koh-product-variant <#if defaultSwatchId == itemSwatchId> koh-selected-variant </#if>" data-koh-image="${skuInitialViewUrl}" data-koh-image-zoom="${skuZoomViewUrl}" data-koh-image-expand="${skuExpViewWithoutZoomUrl}" data-koh-image-expand-zoom="${skuExpViewWithZoomUrl}" data-koh-sku="${skuItem.skuId}" data-koh-sku-k="K-${skuItem.skuId}" data-koh-color="${skuItemColorFinishName}" data-koh-price="<#if productPriceNumber?? && productPriceNumber?has_content>${currencyCode} ${(productPriceNumber?round)*productMultiplication}</#if>" data-default-category="${defaultCategory}"  data-koh-retailer="${retailerMapString}" data-koh-bigcommerce="${bigcommerceUrl}">
		                  		...
		                  		<#if  ( skuItem.discontinued?? && skuItem.discontinued?has_content ) || ( product.discontinued?? && product.discontinued?has_content )>	
									...
									<li>
										...
										<#else>	
											...
											<span class="koh-product-variant<#if defaultSwatchId == itemSwatchId> koh-selected-variant</#if>" data-koh-image="${skuInitialViewUrl}" data-koh-image-zoom="${skuZoomViewUrl}" data-koh-image-expand="${skuExpViewWithoutZoomUrl}" data-koh-image-expand-zoom="${skuExpViewWithZoomUrl}" data-koh-sku="${skuItem.skuId}" data-koh-sku-k="K-${skuItem.skuId}" data-koh-color="${skuItemColorFinishName}" data-koh-itemno="${itemNo}"  data-default-category="${defaultCategory}" data-koh-retailer="${retailerMapString} data-koh-bigcommerce="${bigcommerceUrl}"">
										</#if>
											<button class="koh-product-color">
	        ...
	        <#if locale == "TH">
	        	<div id="bigCommerceContactFormURL">
	        		<#assign retailer4 = "" />
		         	<#if retailerUrlMapDefaultSku?? && (retailerUrlMapDefaultSku?size > 0)>
		         		<#list retailerUrlMapDefaultSku?keys as imageMapKey>
		         			<#if imageMapKey == "RETAILER_4" &&
		         				(retailerUrlMapDefaultSku[imageMapKey]?? && retailerUrlMapDefaultSku[imageMapKey]?has_content)>
		         					<#assign retailer4 = retailerUrlMapDefaultSku[imageMapKey] />
		         			</#if>
			         	</#list>
		         	</#if>
         			<#if retailer4 != "" >
			         	<div class="koh-product-locator">
			            	<a href="${retailer4}" target="_blank" rel="noopener noreferrer">${buyNow}</a>
			         	</div>
		         	<#else>
		         		<div class="koh-product-locator">
	         ...
	         <#else>
	         	...
	         	<#--<div class="koh-product-tools">
				  	<div class="koh-product-share koh-th-share">
			   		  	<div class="koh-share-title">${share}:</div>
			   			<ul class="koh-product-popover">
			          	   <li>
			                 <button class="koh-share-facebook"><i class="fa fa-facebook"></i></button>
			               </li>
			   			</ul>
			    	 </div>
				  </div>-->
		         <div class="koh-product-tools">
		            ...
		            <a data-href="${addToCompareLink}" class="koh-compare-removes koh-compare-button-tile remove_${product.itemNo}" data-product-itemno="${product.itemNo}"   data-category="<#if categoryKey?has_content>${categoryKey}</#if>"  data-product-itemno-multianscestors="${itemNoForCompare}">${compare}</a>
	   ...
	   <div class="koh-product-content">
	      ...
	      <div class="koh-product-features">
	         <div class="koh-product-features-description-files">
	            <#if narrativeDescription??>
	            	<#if locale == "TH">
	            		<h2 class="koh-product-features-title">Information</h2>
	           	 		<div class="koh-product-long-description">${narrativeDescription}</div>
           	 		<#else>
           	 			<div class="koh-product-long-description">${narrativeDescription}</div>
	           	 	</#if>
	      ...
	      <#if locale == "TH">
        	<#if retailerUrlMapDefaultSku?? && (retailerUrlMapDefaultSku?size > 0) >
				<div class="c-koh-product-video-carousel v-koh-inline koh-banner-thumbnail-comp">
					<#list retailerUrlMapDefaultSku?keys as imageMapKey>
						<#if imageMapKey == "RETAILER_6">
							...
							<div class="koh-carousel koh-theme-dark">
								<div class="koh-slide-collection">
									<#if retailerUrlMapDefaultSku[imageMapKey]?? && retailerUrlMapDefaultSku[imageMapKey]?has_content>
										<#list retailerUrlMapDefaultSku[imageMapKey]?split(",") as videoURLs>
											<div class="koh-banner-slide v-koh-simple m-koh-linked">
												<div class="koh-banner-background">
				                                  	<iframe class="koh-shop-by-look-video" src="${videoURLs}" frameborder="0" allowfullscreen></iframe>
												</div>
											</div>
										</#list>
	                          		</#if>
	   ...
	   <#if product.crossSellingDetail.crossSellingAccessories??>
		   <#if listsize &gt; 0>
			   <div class="koh-product-pairs-with">
			      <h2 class="koh-product-pairs-title">${pairsWell}</h2>
					         ...
					         <div class="koh-product-tile">
					            ...
					            <div class="koh-product-tile-content">
					               <a href="${productDetailslink}/${crossSelling.itemNo}?skuid=${defaultSku}">
										...
										<span class="koh-product-sku" data-product-sku="${defaultSku}">K-${defaultSku}</span>
									</a>
					            </div>
					         </div>
				         </#list>
	   ...
	   <#if locale == "TH">
	   		<div class="koh-pdp-promo-banner">
		   		<a href="${promoBannerURL}"  target="_blank">
		   			<img src="${promoBanner}">
	   			</a>
			</div>
	   </#if>
...
<@hst.headContribution category="meta-tags">
		<#if product?? && product.adCopy.metaDescription?has_content>
