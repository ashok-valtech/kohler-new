...
<@fmt.message key="contactFormHrLine" var="contactFormHrLine"/>
<@fmt.message key="promo-home-address-tiltle" var="homeAddressTitle"/>
<@fmt.message key="home-address-error-message" var="homeAddressErrorMessage"/>
...
<input type="hidden" id="homeAddressTitleID" value="${homeAddressTitle}" />
<input type="hidden" id="homeAddressErrorMessageID" value="${homeAddressErrorMessage}" />

<section class="c-koh-contact-form">
    <div class="koh-contact-container">
    <#if contactFormHrLine?? && contactFormHrLine?has_content>
        ...
        <div class="koh-contact-form-wrapper">
            <form method="POST" class="form-horizontal" action="${englishSite}/api/landingpageservice" enctype="multipart/form-data" id="contactUsPromoForm" autocomplete="off">
                    ...
                    <#if homeAddressTitle?? && homeAddressTitle?has_content>
	                    <div class="form-group">
	                        <label class="control-label col-sm-5" for="home-adress-type">${homeAddressTitle}</label>
	                        <div class="col-sm-4">          
	                            <input type="text" class="form-control" id="koh-home-adress-type" maxlength="60" placeholder="" name="koh-home-adress-type">
	                        </div>
	                    </div>
                    </#if>
                    <div class="form-group">
                ...
                <#if storeAndPackageBean??>
            		<#if storeAndPackageBean.kohler_packagecompound?? && storeAndPackageBean.kohler_packagecompound?has_content>
	                    <div class="form-group">
	                        ...
	                        <div class="col-sm-4">
	                           <select class="form-control" id="koh-contact-request-type" name="koh-contact-request-type">
	                                <option value="">${select}</option>
	                                <#list storeAndPackageBean.kohler_packagecompound as packageList>
    									<#if packageList.packageNames?? && packageList.packageNames?has_content>
                            				<option value="${packageList.packageNames}">${packageList.packageNames}</option>
                            			</#if>	
                					</#list>
	                            </select>
	                        </div>
...
<@hst.headContribution category="ext-scripts">
    <script type="text/javascript" src="${link}/js/custom/koh-landing-page-common.min.js"></script>
