...
<@fmt.message key="mu-contact-form-reason" var="reason"/>
<@fmt.message key="mu-interested-product-drop-down-value1" var="intPd1"/>
<@fmt.message key="mu-interested-product-drop-down-value2" var="intPd2"/>
<@fmt.message key="mu-interested-product-drop-down-value3" var="intPd3"/>
<@fmt.message key="mu-interested-product-drop-down-value4" var="intPd4"/>
<@fmt.message key="mu-interested-product-drop-down-value5" var="intPd5"/>
<@fmt.message key="mu-interested-product-drop-down-value6" var="intPd6"/>

<@fmt.message key="mu-isfacebookfield" var="mufacebookfield"/>
<@fmt.message key="mu-contact-form-facebook" var="facebookName"/>
            ...
            <div class="koh-contact-form-wrapper">
               <form class="form-horizontal"  action="<@hst.actionURL escapeXml=false />" method="post" name="${form.name!}" id="mucontactUsForm"
                 <#if form.multipart>enctype="multipart/form-data"</#if> autocomplete="off" >
                     ...
                     <div class="form-group">
                        ...
                        <div class="col-sm-4">
                           <select class="form-control" id="koh-contact-interestedproducts" name="interestedproducts">
                           			<option value="">${select}</option>
                                    <option value="${intPd1}" >${intPd1}</option>
									<option value="${intPd2}" >${intPd2}</option>
									<option value="${intPd3}" >${intPd3}</option>
									<option value="${intPd4}" >${intPd4}</option>
									<option value="${intPd5}" >${intPd5}</option>
									<option value="${intPd6}" >${intPd6}</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        <label class="control-label col-sm-5" for="koh-contact-reason">${reason}</label>
                        <div class="col-sm-4">          
                            <textarea class="form-control" rows="5" id="koh-contact-reason" name="reason" maxlength="4000"></textarea>
                        </div>
                    </div>
                    
                    <#if mufacebookfield?? && mufacebookfield?has_content>
                     <div class="form-group">
                        <label class="control-label col-sm-5" for="koh-contact-facebook">${facebookName}</label>
                        <div class="col-sm-4">          
                            <input type="text" class="form-control" id="koh-contact-facebook" placeholder="" name="facebook" maxlength="20">
                        </div>
                    </div>
                       <#else>
                         <input type="hidden" class="form-control" id="koh-contact-facebook" placeholder="" name="facebook" maxlength="20" value="">
                       </#if>
