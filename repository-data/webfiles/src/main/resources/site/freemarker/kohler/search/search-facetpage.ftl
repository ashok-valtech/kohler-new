...
<div class="koh-search-hero">
	 <div class="koh-hero-container">
		...
		<#if didyoumean?? && didyoumean?has_content>
			<p>
			  	<h1>  ${didyoumeanquestion}  <a href="<@hst.link siteMapItemRefId="results" />?search=${didyoumean?html}&currentpage=1<#if orderBy?? && orderBy?has_content>&orderBy=${orderBy?html}</#if><#if sort?? && sort?has_content>&sort=${sort?html}</#if>">${didyoumean?html} <span></span></a>&nbsp &nbsp </h1>
			<p> 	
		</#if>
	        ...
	        <ul class="koh-search-set">
	          <#if totalProductCount?? && (totalProductCount gt 0)>
				 <li <#if type == "product">class="active" </#if>>
				 	 <a href="<@hst.link siteMapItemRefId="results" />?search=${search?html}&type=product&currentpage=1<#if orderBy?? && orderBy?has_content>&orderBy=${orderBy?html}</#if><#if sort?? && sort?has_content>&sort=${sort?html}</#if>">${searchproducts?html} <span>(${totalProductCount?html})</span></a>&nbsp &nbsp
				 </li>	  	
			  </#if>
			  <#if totalSpecCount?? && (totalSpecCount gt 0)>
				 <li <#if type == "spec">class="active" </#if>>
				 	 <a href="<@hst.link siteMapItemRefId="results" />?search=${search?html}&type=spec&currentpage=1<#if orderBy?? && orderBy?has_content>&orderBy=${orderBy?html}</#if><#if sort?? && sort?has_content>&sort=${sort?html}</#if>">${searchspec?html} <span>(${totalSpecCount?html})</span></a>&nbsp &nbsp
				 </li>	  		 	  	
			  </#if>  
			   <#if totalArticleVideoSize?? & totalArticleVideoSize gt 0>
				 <li <#if type == "article" || type == "pressreleases" || type == "video">class="active" </#if>>
				 	 <a href="<@hst.link siteMapItemRefId="results" />?search=${search?html}&type=article&currentpage=1<#if orderBy?? && orderBy?has_content>&orderBy=${orderBy?html}</#if><#if sort?? && sort?has_content>&sort=${sort?html}</#if>">${searcharticleandvideo?html} <span>(${totalArticleVideoSize?html})</span></a>&nbsp &nbsp
				 </li>
				 	  		 	  	
			  </#if>  
			     <#if totalHelpCount?? && (totalHelpCount gt 0)>
				 <li <#if type == "help">class="active" </#if>>
				 	 <a href="<@hst.link siteMapItemRefId="results" />?search=${search?html}&type=help&currentpage=1<#if orderBy?? && orderBy?has_content>&orderBy=${orderBy?html}</#if><#if sort?? && sort?has_content>&sort=${sort?html}</#if>">${searchhelp?html}<span>(${totalHelpCount?html})</span></a>&nbsp &nbsp
				 </li>
