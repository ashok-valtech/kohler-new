				...
				<div class="col-sm-6 koh-150-anniversary-cta-image-section-title">
					<div class="koh-150-anniversary-cta-image-section-inner-title">
						...
						<#if title?? && title?has_content>
							<div class="koh-150-anniversary-cta-image-section-first-title">${title}</div>
						</#if>
                        <#if walkInBathFBDetails.description.content?? && walkInBathFBDetails.description.content?has_content>
							<div class="koh-150-anniversary-cta-image-section-description"><@hst.html hippohtml=walkInBathFBDetails.description/></div>
						</#if>
						<#if walkInBathFBDetails.imageCaption?? && walkInBathFBDetails.imageCaption?has_content>
							<a href="${navigationURL}" target="${target}" class="koh-150-anniversary-cta">${walkInBathFBDetails.imageCaption}</a>
						</#if>
					</div>
				</div>
			</div>
