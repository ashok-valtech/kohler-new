...
<@fmt.message key="select-market" var="selectMarket"/>
<@fmt.message key="all-products" var="allProducts"/>
<@fmt.message key="cookie-name-re-market" var="cookieNameReMarket"/>
<@fmt.message key="cookie-name-re-market-duration" var="durationCookieReMarket"/>
...
<input id="cookie-info-re-market" type="hidden" data-cookie-duration="${durationCookieReMarket}" data-cookie-name="${cookieNameReMarket}"/>
<#assign locale = hstRequest.getLocale().getLanguage() />
<#assign locale = locale + "_"+ hstRequest.getLocale().getCountry() />
<header>
	<#if isCollections?? && isCollections?has_content>
	...
	<div class="koh-search-sorting <#if marketList??>koh-market-availability-sort-by</#if>">
	...
	<div class="koh-search-filters">
		...
		<div class="koh-search-filters-content">
			<div class="koh-filter-group koh-category <#if section == "FacetPage">koh-category-hidden</#if>">
					...
					<ul class="koh-selected-filters">
						<li>
							<button class="koh-selected-filter" data-facet-name="Categories" data-facet-value="${categories}/${leafNode.name}" data-facet-count="<#if leafNode??>${leafNode.count}</#if>">
								...
								<span class="name">
									<#if currentCategory??>
										<#assign categoryName = currentCategory.getInfo.getName() />
										<#if categoryName??>
					                     	${categoryName}
					                 	</#if>
									</#if>
...
<#if marketList??>
		<input name="select-market-label" id="select-market-label" type="hidden" value="${selectMarket}" />
		<div class="koh-market-availability">
			<div class="koh-market-title">Market Availability</div>
			<span class="koh-market-list-close-button"></span>
			<div>
				<#if selectedCountry??>
					<@fmt.message key="${selectedCountry.name}" var="selectedCountryName"/>
					<span>
						<input id="koh-allproduct-values" type="radio" name="market" value="${allProducts}">
						<label for="koh-allproduct-values">${allProducts}</label>
					</span>
					<span class="koh-market-values">
						<input id="koh-market-values" type="radio" name="market" value="${selectedCountry.name}" checked>
						<label class="koh-market-values-click" for="koh-market-values">${selectedCountryName}<span class="koh-market-drop"></span></label>
		  			</span>
		  		<#else>
		  			<span>
						<input id="koh-allproduct-values" type="radio" name="market" value="${allProducts}" checked>
						<label for="koh-allproduct-values">${allProducts}</label>
					</span>
					<span class="koh-market-values">
						<input id="koh-market-values" type="radio" name="market" value="${selectMarket}"  \>
						<label class="koh-market-values-click" for="koh-market-values">${selectMarket}<span class="koh-market-drop"></span></label>
					</span>
		  		</#if>		
			</div>
			<div class="market-list">
				<h2> Location </h2>
				<div class="row">
					<@fmt.message key="SKU_LOGO_BASE_URL" var="logoBaseUrl"/>
					<@fmt.message key="SKU_LOGO_BORDER" var="logoBorderSkus"/>
					<#list marketList.folders as market>
						<@fmt.message key="${market.name}" var="marketName"/>
						<div class="col-md-4 col-sm-4 col-xs-12">
							<a class="koh-country-name <#if selectedCountry?? && selectedCountry.name == market.name>koh-market-selected</#if>" href="" data-facet-label="${marketName}" data-facet-name="${marketList.name?html}" data-facet-count="${market.count}" data-facet-value="${market.name}">
								<span class="market-name"><span><img src="${logoBaseUrl}${market.name?lower_case}.png" <#if logoBorderSkus?contains(market.name)>class="koh-country-logo-border"</#if>/></span>${marketName}
							</a>
						</div>		
					</#list>
				</div>
