...
var homeAddressTitleID = $('#homeAddressTitleID').val();
var homeAddressErrorMessageID = $('#homeAddressErrorMessageID').val();
...
$(document).ready(function () {
    $('#contactUsPromoForm').submit(function (e) {
        e.preventDefault();
...
function validateContactForm(contactEmailLink) {
    var packageTypeVal = $('#koh-contact-request-type').val();
    ...
    if(homeAddressTitleID != '' && homeAddressTitleID != null){
	    if (homeAddress == '' || homeAddress == null) {
	        isValid = false;
	        showError('#koh-home-adress-type', homeAddressErrorMessageID);
	    }
	}
    ...
    if (packageTypeVal == '') {
        isValid = false;
        showError('#koh-contact-request-type', selectRamadanPackageErrorMessage);
    }
    ...
    if (isValid == true) {
        ...
        fdata.append("koh-contact-request-type", packageTypeVal);
        ...
        fdata.append("koh-home-adress-type", homeAddress);
        $.ajax({
...
$(document).on('keyup change', '#contactUsPromoForm input, #contactUsPromoForm select', function (e) {
    ...
    if(homeAddressTitleID != '' && homeAddressTitleID != null){
	    if (currentIdName == 'koh-home-adress-type') {
	        ...
	        if (currentVal == "") {
	            isValid = false;
	            showError(currentId, homeAddressErrorMessageID);
	        } else {
	            isValid = true;
	            removeError(currentId);
	        }
    ...
    if (currentIdName == 'koh-contact-request-type') {
        ...
        if (currentVal == "") {
            isValid = false;
            showError(currentId, selectRamadanPackageErrorMessage);
        } else {
            isValid = true;
            removeError(currentId);
        }
