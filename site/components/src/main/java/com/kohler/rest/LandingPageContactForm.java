...
import com.kohler.commons.components.EmailServiceComponent;
...
@Path("/contactpromotionform/")
public class LandingPageContactForm extends BaseRestResource {

	ResourceBundleCustom resourceBundle = new ResourceBundleCustom();
	private static final Logger LOGGER = LoggerFactory.getLogger(LandingPageContactForm.class);
	...
	public Map<String, String> createContactForm(@Multipart("koh-contact-request-type") String packageType,
		...
		String content = null;
		try {
			content = new String(Files.readAllBytes(readfile.toPath()));
			...
			content = content.replace("${packageTypeValue}", requestTypeLabel);
			content = content.replace("${districtType}", contactFormTempalteDistrictsType);
			content = content.replace("${districtTypeValue}", districtTypeLabel);
		...
		try {
			userContent = new String(Files.readAllBytes(readUserTempfile.toPath()));
