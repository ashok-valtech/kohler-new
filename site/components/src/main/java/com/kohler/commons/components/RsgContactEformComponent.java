 ...
 * @version 1.0
 */
public class RsgContactEformComponent extends FormStoringEformComponent {
	...
	public void doAction(HstRequest request, HstResponse response) throws HstComponentException {
		...
		try {
			...
			farwardPage = resourceBundle.getPropertyValue(mu_bundle, Constants.RSG_FORM_FARWARPAGE);
			...
			new Thread() {
				public void run() {
					...
					String muUserEmailTemplate = resourceBundle.getPropertyValue(mu_bundle,"rsg-promotion-user-email-template");
					userContent = muUserEmailTemplate;
					String userTemplateBannerImage = resourceBundle.getPropertyValue(mu_bundle, "rsg-email-template-banner");
					String userTemplateThankYouMessage = resourceBundle.getPropertyValue(mu_bundle, "mu-userTemplateThankyou");
					String userTemplateDesc1Message = resourceBundle.getPropertyValue(mu_bundle, "rsg-email-template-description1");
					String userTemplateDesc2Message = resourceBundle.getPropertyValue(mu_bundle, "rsg-email-template-description2");
					String userTemplateSubscribeMessage = resourceBundle.getPropertyValue(mu_bundle, "rsg-userTemplateSubscribe");	
					String userTemplateUnsubscibeEmail = resourceBundle.getPropertyValue(mu_bundle, "rsg-userTemplateUnsubscribeEmail");
					String regards = resourceBundle.getPropertyValue(mu_bundle, "mu-userTemplateRegards");
					String regardName = resourceBundle.getPropertyValue(mu_bundle, "rsg-userTemplateRegardsName");
					String userTemplateDearLable = resourceBundle.getPropertyValue(mu_bundle, "mu-userTemplateDear");
					String userTemplateFacebookURL = resourceBundle.getPropertyValue(mu_bundle, "rsg-userTemplateFacebookURL");
					String userTemplateInstagramURL = resourceBundle.getPropertyValue(mu_bundle, "rsg-userTemplateInstagramURL");
					...
					String userTemplatePdfUrl = resourceBundle.getPropertyValue(mu_bundle, "rsg-userTemplatePdfUrl");
					String mupromotionlink = resourceBundle.getPropertyValue(mu_bundle, "rsg-promotionlink");
					...
					String muUserTemplateSubject = resourceBundle.getPropertyValue(mu_bundle, "rsg-userTemplateSubject");
			...
			try {
				System.out.println("Farward to KYR Thankyou page");
